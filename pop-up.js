/******************************************************************************************************
               Paradox dé(s)matérialité(s) du numérique à l'aire post-ditiale ecrit par Thomas FERREIRA
                           Réalisé entre 2021-2023
*******************************************************************************************************

Please see https://ferreirathomasarts.wordpress.com/ for details and a demo of this script
Please see https://ferreirathomasarts.wordpress.com/ for terms of use
...
// Nouvelle fonction pour définir le chemin du fichier HTML
function setHtmlFilePath(path) {
    window.htmlFilePath = path;
}
...
function createMiniWinLayer(mainContent, oTitle, oLeft, oTop, oWidth, barColour, mainColour, oLogo, oMin, oMax, oClose, oFull, oDrag, oVis, oFullDrag, anyExtra, oScroll) {
    if (typeof(oFullDrag) == 'undefined') {
        window.alert('New mini window script incompatible.\nPlease open the website from another Web Browser.');
        return;
    }
    ...
    xmlhttp.onreadystatechange = function () {
        if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
            ...
            document.getElementById('MWJminiwinMAX' + window.MWJ_MINIWINS).innerHTML = content;
        }
    };
    // Utilisez window.htmlFilePath ici
    xmlhttp.open("GET", window.htmlFilePath, true);
    xmlhttp.send();
    } else {
    ...
    tempOb.maxName = 'MWJminiwinMAX' + window.MWJ_MINIWINS;
...
function clickDiv(div1, div2, e, isRes) {
    ...
    if ((!window.ActiveXObject || navigator.userAgent.indexOf('Mac') == -1 || navigator.__ice_version) && document.onmousemove == winIsMove) {
    ...
    MWJ_winZind += 2;
    div1 = getRefToDivNest(div1);
    div2 = getRefToDivNest(div2);
    if (!div2) {
        return;
    }
    window.msStartCoord = winMousePos(e);
    ...
    if (div1.style) {
        div1.style.zIndex = MWJ_winZind;
        div2.style.zIndex = MWJ_winZind + 1;
    } else {
        div1.zIndex = MWJ_winZind;
        div2.zIndex = MWJ_winZind + 1;
    }
    ...
    document.onmousemove = winIsMove;
    document.onmouseup = winIsMove;
}

function winIsMove(e) {
    //move the layer to its newest position
    var msMvCo = winMousePos(e);
    if (!e) {
    ...
    if (navigator.product == 'Gecko') {
        window.setTimeout('winIsMove2(' + newX + ',' + newY + ');', 1);
    } else {
        winIsMove2(newX, newY);
    }
}
...
function doActualResize(oDivs, y, z, offSet) {
    ...
    for (var x = 0; x < 2; x++) {
        ...
        if (!x && theLayer.getElementsByTagName('div')[1].style.overflow == 'auto' && navigator.product == 'Gecko' && navigator.taintEnabled) {
            ...
            if (!ppDv.tmpDiv) {
                ppDv.tmpDiv = document.createElement('div');
        ...
        if (!x) {
            ...
            if (theLayer.getElementsByTagName('div')[1].tmpDiv) {
                ...
                ppDv.style.height = doNumMax(96 + (ppDv.parentNode.offsetHeight / 500)) + '%';
            }
        }
    }
}

function doNumMax(oNm) {
    return (oNm > 99) ? 99 : oNm;
}

function moveWin(oMax, oMin, x, y) {
    oMax = getRefToDivNest(oMax);
    oMin = getRefToDivNest(oMin);
    if (!oMax) {
        return;
    }
    if (oMax.style) {
...
function resizeWin(oMax, oMin, y, z) {
    oMax = [getRefToDivNest(oMax), getRefToDivNest(oMin)];
    if (!oMax[0]) {
        return;
    }
...
function maximiseWin(oMax, oMin) {
    var div1 = getRefToDivNest(oMax),
        div2 = getRefToDivNest(oMin);
    if (!div2) {
        return;
    }
    MWJ_winZind += 2;
...
function findMaxSize() {
    ...
    if (window.opera || (navigator.product == 'Gecko' && navigator.taintEnabled)) {
        w -= 16;
    }
    if (!window.ActiveXObject && !navigator.taintEnabled) {
        h -= 4;
...
function hideMinWin(thisDiv) {
    ...
    thisDiv = getRefToDivNest(thisDiv);
    if (!thisDiv) {
        return;
    }
    if (thisDiv.style) {
...
function showMinWin(thisDiv, thatDiv) {
    ...
    if (thisDiv.style) {
        ...
        thisDiv.style.zIndex = MWJ_winZind;
    } else {
        thisDiv.visibility = 'show';
        thisDiv.zIndex = MWJ_winZind;
    }
}

function manuf() {
    window.alert('Fenêtre Pop-up\nWritten by Thomas FERREIRA, 2023\n' +
        '\nSee https://ferreirathomasarts.wordpress.com/.\n' +
        '\nSee https://ferreirathomasarts.wordpress.com/ for detailed support info and other scripts');
}